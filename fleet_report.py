...
def car_wear(car: dict) -> float:
    """Return the wear percentage for one car (0–100+).

    Falls back to the car's own odometer reading when last_service_km is
    absent, matching the same safe default used in needs_service.
    """
...
def fleet_summary(fleet: list[dict]) -> dict:
    """Return a dict with count, due count, and average wear for the fleet."""
    total = 0.0
    due = 0
    for car in fleet:
        total += car_wear(car)
...
def print_report(fleet: list[dict]) -> None:
    """Log and print the nightly fleet-health report."""
